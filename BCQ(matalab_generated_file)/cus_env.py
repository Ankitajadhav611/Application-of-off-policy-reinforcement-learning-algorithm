...
class CustomEnv(gym.Env):
        ...
        def __init__(self):
            ...
            self.output_ax1.set_ylim(-15,15)
            self.output_ax1.set_title('output of state space yout_1')
            self.output_ax2.set_ylim(-15,15)
            self.output_ax2.set_title('output of state space yout_2')
            self.input_ax.set_ylim(-170,170)
            self.input_ax.set_title('input to state space(action)')
            self.reward.set_ylim(-1.5,1.5)
            self.reward.set_title('Generated reward')
            self.reward.set_xlabel('time (s)')
            self.reward.set_ylabel('')
        ...
        def reward_function(self, y_out):
            e1 = 1/20*abs(self.y_ref[0, 0] - y_out[0])
            e2 = 1/20*abs(self.y_ref[1, 0] - y_out[1])
            reward = 1 - (0.5*(e1+e2))
            return reward
        ...
        def reset(self):
            ...
            self.output_line_1r.set_ydata(self.y_ref[0,:])
            self.output_line_2.set_ydata(self.y[1,:])
            self.output_line_2r.set_ydata(self.y_ref[1,:])
        ...
        def step(self, action):
            ...
            self.T = np.array([self.T[-1], self.T[-1]+ 0.01])
